...
<form  action="{{ url('/roles/grupos/'. $rol) }}" method="POST"  class="row justify-content-around">
    @csrf
    <div class="col-6">
        <x-adminlte-select2 name="user"  igroup-size="md"
        data-placeholder="Select un usuario...">
            ...
            @foreach ($userHasRoles as $userHasRole)
                <tr>
                    ...
                    <td><img src="{{ asset('fotos-de-usuarios/' . $userHasRole->personal->img_url) }}"
                            class="img-circle elevation-2" width="50px" height="50px"
                            alt="User Image"></td>
                    <td>
                        <form action="{{ url('/roles/grupos/' . $rol->id) }}" class="form-delete"
